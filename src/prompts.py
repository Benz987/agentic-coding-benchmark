...
First, write a strict "Software Design Document" under the header ### MANIFEST ###. Define exact class names, method signatures, variable names, and data structures to prevent integration errors.
...
CRITICAL INSTRUCTION: If you receive ### CRITIC FEEDBACK ### from a previous failed execution, use that insight to adjust your manifest and subtask breakdown.

Your response MUST contain the ### MANIFEST ### block, followed by a valid JSON list of strings containing the subtask descriptions. 
CRITICAL CONSTRAINT: The JSON list MUST be wrapped inside a ```json ... ``` code block. Do not include markdown formatting outside the manifest and the JSON block."""
...
CRITICAL CONSTRAINT: You must output ONLY valid Python code. Do NOT use HTML/XML tags (like <!-- -->) for comments, only use standard Python hashes (#).
...
If you receive ### CRITIC FEEDBACK ###, it means there was a minor syntax error in a previous run. Apply the bug fixes described in the feedback to the merged code.
CRITICAL CONSTRAINT: You must output ONLY valid Python code. Do NOT use HTML/XML tags (like <!-- -->) for comments, only use standard Python hashes (#).
